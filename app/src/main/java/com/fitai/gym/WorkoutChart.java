...
public class WorkoutChart extends View {
    ...
    private void init() {
        ...
        gridPaint.setColor(0x33FFFFFF); // Semi-transparent white
        ...
        line2Paint.setColor(0x66FFFFFF); // more transparent
        ...
        dotPaint.setColor(0xFFAECBFA); // Light blue dot
    ...
    protected void onDraw(Canvas canvas) {
        ...
        float paddingRight = 120f; // space for Y labels
        float paddingTop = 40f;
        float paddingBottom = 80f; // space for X labels
        ...
        for (int i = 0; i < yLabels.length; i++) {
            ...
            Paint tp = (i == 2) ? boldTextPaint : textPaint; // Make 40% bold as in image
        ...
        float stepX = chartWidth / (xLabels.length - 1);
        for (int i = 0; i < xLabels.length; i++) {
            float x = paddingLeft + (i * stepX);
            Paint tp = (i == highlightIndex) ? boldTextPaint : textPaint;
            canvas.drawText(xLabels[i], x, height - 20f, tp);
        }

        // Draw highlight bar
        float highlightX = paddingLeft + (highlightIndex * stepX);
        float highlightY1 = height - paddingBottom - (data1[highlightIndex] / 100f * chartHeight);
        float highlightY2 = height - paddingBottom - (data2[highlightIndex] / 100f * chartHeight);
